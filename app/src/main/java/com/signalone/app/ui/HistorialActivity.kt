...
class HistorialActivity : AppCompatActivity() {
    ...
    private fun renderHistorial() {
        ...
        if (AppState.historial.isEmpty()) {
            ...
            b.llAlertas.addView(tv)
        } else {
            AppState.historial.forEach { alerta -> b.llAlertas.addView(buildCard(alerta)) }
        }
    }
    ...
    private fun buildCard(a: AlertaHistorial): LinearLayout {
        ...
        val contactosTexto = "👥 ${a.contactosNotificados.joinToString(", ")}"
        col.addView(tvSecund(contactosTexto, 12f, dp))

        // Ubicación — tappable si hay URL
        if (a.ubicacionUrl != null) {
            val tvUbi = TextView(this).apply {
